...
def get_full_table_info(parsed_sql):
    """
    Returns a dictionary mapping table aliases to their full names
    """
    table_info = {}
    for table in parsed_sql.find_all(exp.Table):
        if table.alias:
            table_info[table.alias] = {
...
table_info = get_full_table_info(parsed_sql)

print("="*80)
print("Column Lineage Analysis:")
print("="*80)

# Print table information first
print("\nTable Information:")
    ...
    try:
        ...
        if column_refs:
            for col in column_refs:
                ...
                full_table_info = table_info.get(table_alias, {'full_name': 'Unknown', 'schema': None, 'name': None})

                print(f"  Table: {full_table_info['full_name']} (Alias: {table_alias})")
